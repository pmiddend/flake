...
//_____________________________________________________________________________
// Constructor
MarchingCubes::MarchingCubes( const int size_x_param /*= -1*/, const int size_y_param /*= -1*/, const int size_z_param /*= -1*/ ) :
//-----------------------------------------------------------------------------
  _originalMC(false),
  _ext_data  (false),
  _size_x    (size_x_param),
  _size_y    (size_y_param),
  _size_z    (size_z_param),
  _data      (0),
  _x_verts   (0),
  _y_verts   (0),
  _z_verts   (0),
...
//-----------------------------------------------------------------------------
{
  ...
  memset( _z_verts, -1, static_cast<size_t>(_size_x * _size_y * _size_z * static_cast<int>(sizeof( int ))) ) ;
}
//_____________________________________________________________________________
...
//-----------------------------------------------------------------------------
{
  ...
  if( At >= 0 ) test ++ ;
  if( Bt >= 0 ) test = static_cast<char>(test+2) ;
  if( Ct >= 0 ) test = static_cast<char>(test+4) ;
  if( Dt >= 0 ) test = static_cast<char>(test+8) ;
  switch( test )
  {
  case  0 : return s>0 ;
  case  1 : return s>0 ;
  ...
  case  3 : return s>0 ;
  case  4 : return s>0 ;
  case  5 : if( At * Ct - Bt * Dt <  static_cast<real>(FLT_EPSILON) ) return s>0 ; break ;
  case  6 : return s>0 ;
  case  7 : return s<0 ;
  case  8 : return s>0 ;
  case  9 : return s>0 ;
  case 10 : if( At * Ct - Bt * Dt >= static_cast<real>(FLT_EPSILON) ) return s>0 ; break ;
  case 11 : return s<0 ;
  case 12 : return s>0 ;
  case 13 : return s<0 ;
  case 14 : return s<0 ;
...
//-----------------------------------------------------------------------------
{
  ...
  case  7 :
	  if( test_face( test7[_config][0] ) ) _subconfig =  static_cast<uchar>(_subconfig+1) ;
    if( test_face( test7[_config][1] ) ) _subconfig =  static_cast<uchar>(_subconfig+2) ;
    if( test_face( test7[_config][2] ) ) _subconfig =  static_cast<uchar>(_subconfig+4) ;
    switch( _subconfig )
      {
      case 0 :
  ...
  case 13 :
    if( test_face( test13[_config][0] ) ) _subconfig = static_cast<uchar>( _subconfig + 1);
    if( test_face( test13[_config][1] ) ) _subconfig = static_cast<uchar>( _subconfig + 2);
    if( test_face( test13[_config][2] ) ) _subconfig = static_cast<uchar>( _subconfig + 4);
    if( test_face( test13[_config][3] ) ) _subconfig = static_cast<uchar>( _subconfig +  8);
    if( test_face( test13[_config][4] ) ) _subconfig = static_cast<uchar>( _subconfig + 16);
    if( test_face( test13[_config][5] ) ) _subconfig = static_cast<uchar>( _subconfig + 32);
...
//_____________________________________________________________________________
// Adding triangles
void MarchingCubes::add_triangle( const char* trig_param, char n, int v12 )
//-----------------------------------------------------------------------------
{
  int    tv[3] ;
...
//-----------------------------------------------------------------------------
{
  ...
  Vertex *vert_param = _vertices + _nverts++ ;
...
//-----------------------------------------------------------------------------
{
  ...
  vert_param->x = vert_param->y = vert_param->z =  vert_param->nx = vert_param->ny = vert_param->nz = 0 ;

  // Computes the average of the intersection points of the cube
  vid = get_x_vert( _i , _j , _k ) ;
  if( vid != -1 ) { ++u ; const Vertex &v = _vertices[vid] ; vert_param->x += v.x ;  vert_param->y += v.y ;  vert_param->z += v.z ;  vert_param->nx += v.nx ; vert_param->ny += v.ny ; vert_param->nz += v.nz ; }
