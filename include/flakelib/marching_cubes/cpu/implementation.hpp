...
public :
  ...
  inline Vertex   * vert( const int i ) const { if( i < 0  || i >= _nverts ) return 0; return _vertices  + i ; }
  /** accesses a specific triangle of the generated mesh */
  inline Triangle * trig( const int i ) const { if( i < 0  || i >= _ntrigs ) return 0; return _triangles + i ; }
  ...
  { if( !_ext_data ) delete [] _data ;  _ext_data = data != 0 ;  if( _ext_data ) _data = data ; }
  /**
   * selects to allocate data
   */
  inline void set_int_data  () { _ext_data = false ;  _data = 0 ; }
